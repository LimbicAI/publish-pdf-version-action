...
async function getTemplateHtml(fileName) {
  ...
  const templateBuffer = await getAsset(fileName).catch((err) =>
    console.log("Can' load template: ", err)
  );
...
async function generatePdf() {
   ...
   * uploading PDF to S3
   */
  await uploadFile(mergedPdf, `versions/${pdfName}`, 'pdf');
  await uploadFile(mergedPdf, `latest/${latestPdfKey}`, 'pdf');
}
